...
# Function to parse input file of mzml format, extract MS1 features and return as a numpy array
def mzml_parser(infile_raw, noise_threshold):
    '''
    args: 
        infile_raw <string>. Path_to_file of input
        threshold <float>. Optional threshold to filter out peaks below a certain intensity.
    ...
    for n, spec in enumerate(parser):
        for j in range(len(spec.centroidedPeaks)):
            if spec.centroidedPeaks[j][1] >= noise_threshold:
...
def PathGen(
    ...
    try:
        if infile_raw.endswith(".mzTab"):
            ...
            data = mzml_parser(infile_raw, 100000)
        else:
    ...
    try:
        logger.info("Step 3")
        logger.info(range(num_path))
        for i in range(num_path):
            edges = EdgeCreate(
                clusters, intensity_accu, num_node, delay, min_scan, max_scan
            )
            logger.info("Step 3b") #### THIS STEP USES A LOT OF MEMORY
