...
def generate_Exploris_list_int(input_table: str, output_filename:str, 
                               pretarget_rt_exclusion_time:float, posttarget_rt_exclusion_time:float,
                               min_int_apex_ratio:float):
    """Format a table with mz, charge, rt, intensities into a Exploris serie inclusion/exclusion list"""
    # Prepare the columns
    df_master = pd.read_csv(input_table)
    df_master['Start [min]']=(df_master['retention_time']-(pretarget_rt_exclusion_time))/60
    ...
    df['m/z'] = df_master["Mass [m/z]"].round(decimals=4)
    df['z'] = df_master['charge']
    df['Intensity Threshold'] = df_master.iloc[: , -1].astype(int)
    df['Intensity Threshold'] = df['Intensity Threshold']*min_int_apex_ratio
    df["RT Time (min)"] = df_master["block1"]
    df["Window (min)"] = ((pretarget_rt_exclusion_time+posttarget_rt_exclusion_timee)/60)
    df["Window (min)"] = round(df["Window (min)"],4)
    df.to_csv(output_filename, index = False, sep=',')
