...
def IODA_exclusion_workflow(input_mzML,ppm_error,narrow_noise_threshold,large_noise_threshold):
    ...
    if input_mzML.startswith(('http','ftp')):
        ...
        bashCommand5 = "mono ThermoRawFileParser/ThermoRawFileParser.exe -i="+input_mzML+" --logging=1 --ignoreInstrumentErrors --excludeExceptionData --output_file "+os.path.join(OPENMS_folder+"/OpenMS_input/", os.path.basename(input_mzML)[:-3] + ".mzML")
    ...
    if input_mzML.endswith(('.raw','.RAW')):
        try:
            f = open(os.path.join(OPENMS_folder+"/OpenMS_input/", os.path.basename(input_mzML)[:-3] + ".mzML"))
            f.close()
        except subprocess.CalledProcessError:
